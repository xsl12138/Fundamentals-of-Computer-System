...
int rotateLeft(int x, int n) {
  ...
  int y = x;
  x = x << n;
  y = (y >> (32 + (~n) + 1)) & a; //注意32-n = 32+(-n)，-n用补码表示，即n的反码+1
  x = x | y;
...
unsigned float_abs(unsigned uf) { 
  ...
  //if(uf==uf){
    uf=uf&(~(1<<31));
    if(uf>0x7f800000)   //NaN的二进制码：符号位为1或0，阶码全为1，尾数≠0（若尾数全为0则是正负无穷，不是NaN）
      return x;
    return uf;
  //}
  //else
 ...
 *   Rating: 4
 */
int float_f2i(unsigned uf) {  //取出浮点数的阶码部分，若大于158（偏移量127，大于158说明该数超出了2^31），则超出整数范围，若小于127，则整数部分为0；
  unsigned nan=0x80000000;
	int e=uf&0x7F800000;  //取阶码
   	int little=0;
	e=e>>23;
 	if(e>127+31)
		return nan;
	...
	little=(uf&0x007FFFFF);//取出尾数部分，加上隐藏的一位。若阶码部分大于 150（150-127=23，正好是尾数的位数，所以要以150为界），则左移e-150位，否则右移150-e位。 
